...
import type { DeviceAdapter, ParsedDeviceData } from "./core/device-adapter";
...
export class DeviceIntegrationService implements DeviceIntegrationServiceInstance {
  ...
  async syncDevice(userId: string, device: Device): Promise<SyncPreview> {
    let parsed: ParsedDeviceData;
    try {
      parsed = await this.manager.sync(device);
    } catch (error) {
    ...
    const existingKeys = new Set(
      existing.map((record) =>
        record.sourceKey ??
        `${device.adapterId}|${device.id}|${device.transport}|${record.measuredAt}|glucose|${record.value}`
      )
    );
    ...
    const result = buildSyncResult({
      ...
      unique,
      duplicateCount,
      erroredCount: parsed.droppedCount,
      message:
        unique.length === 0
  ...
  async confirmImport(userId: string, preview: SyncPreview): Promise<SyncResult> {
    ...
    return {
      ...outcome.result,
      newRecords: imported,
      errorCount: errors + outcome.result.errorCount,
