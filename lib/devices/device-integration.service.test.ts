...
function makeService(measurements: DeviceMeasurement[]) {
  const adapter: DeviceAdapter = {
    ...
    sync: vi.fn().mockResolvedValue({ measurements, droppedCount: 0 }),
  };
...
describe("DeviceIntegrationService — sync preview", () => {
  ...
  it("reports the parser droppedCount as errorCount in the preview", async () => {
    const adapter: DeviceAdapter = {
      id: "test-glucose",
      name: "Glucose",
      isSupported: () => true,
    ...
    const manager = new DeviceManager();
    manager.register(adapter);
    const preview = await new DeviceIntegrationService(manager).syncDevice(
      TEST_USER_ID,
      device()
    );
    expect(preview.result.errorCount).toBe(3);
  });

  it("deduplicates across syncs when records carry a native source id", async () => {
    const nativeId = glucoseMeasurement({
      source: { ...glucoseMeasurement().source, sourceRecordId: "native-1" },
    });
    const service = makeService([nativeId]);
    await service.confirmImport(TEST_USER_ID, await service.syncDevice(TEST_USER_ID, device()));
