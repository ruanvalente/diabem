...
describe("DeviceManager", () => {
  ...
  it("sync delegates to the adapter and returns parsed data", async () => {
    const manager = new DeviceManager();
    ...
    const sync = vi.fn().mockResolvedValue({ measurements: [measurement], droppedCount: 2 });
    manager.register(mockAdapter({ id: "test", sync }));
    const result = await manager.sync(DEVICE);
    expect(result).toEqual({ measurements: [measurement], droppedCount: 2 });
  });
});
